...
public class CheckInRepositoryTest {
    ...
    public void findByUserIdOrderByIdDescが最新のものを適切に取得できていることの確認() throws Exception {
        testEntityManager.persist( new CheckInEntity(0, "2", 10, null, new Date(), true));
        testEntityManager.persist( new CheckInEntity(0, "1", 10, null, new Date(), true));
        testEntityManager.persist( new CheckInEntity(0, "1", 10,null, new Date(), false));
        testEntityManager.persist( new CheckInEntity(0, "2", 10,null, new Date(), true));
        ...
        assertCheckInEntityWithoutDate(actual.get(0), new CheckInEntity(3, "1", 10,null, null, false));
    }
    ...
    public void チェックアウトメソッド_対象機能のみチェックアウト_対象外もののがチェックアウトされていないこと() {
        ...
        testEntityManager.persist( new CheckInEntity(0, "1", 10,null, new Date(), false));
        testEntityManager.persist( new CheckInEntity(0, "1", 10,null, new Date(), true));
        testEntityManager.persist( new CheckInEntity(0, "2", 10,null, new Date(), false));
        testEntityManager.persist( new CheckInEntity(0, "2", 10,null, new Date(), true));
    ...
    private void assertCheckInEntityWithoutDate(CheckInEntity actual, CheckInEntity expected) {
        ...
        assertThat(actual.getPlaceUserId(), is(expected.getPlaceUserId()));
